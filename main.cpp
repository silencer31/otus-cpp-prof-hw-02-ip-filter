...
int main (int, char **) {
    ...
    try
    {
        // Вектор с ip адресами - векторами строк.
        std::vector<std::vector<std::string> > ip_pool;

        // Чтение списка ip адресов из тестового файла.
        if (!get_ip_list_from_file(ip_pool, std::string{ "ip_filter.tsv" })) {
            return EXIT_FAILURE;
        }
   
        // Вывод в консоль оригинального списка.
        ...
        // Вывод в консоль списка, отсортированного в обратном порядке.
        if (ask_to_print("\nWould you like to see reversed ip list?")) {
            print_ip_vector(ip_pool);
        }

        // Вывод в консоль отсортированного списка, но только с 1 в первой октете.
        if (ask_to_print("\nWould you like to see filtered by first byte ip list?")) {
            filter_output(ip_pool, std::vector<int> {1});
        }

        // Вывод в консоль отсортированного списка, но только со значением 46 в первой октете и 70 во второй.
        if (ask_to_print("\nWould you like to see filtered by first and second bytes ip list?")) {
            filter_output(ip_pool, std::vector<int> {46, 70});
        }

        // Вывод в консоль отсортированного списка, но только со значением 46 в любой октете.
